...
function camera.create_camera(point, focal_length, focal_height, yaw, pitch, range_of_visibility)
    local id = game_api.add_camera_conf(point.base(), Fix32(focal_length), Fix32(focal_height), Fix32(yaw), Fix32(pitch), Fix32(range_of_visibility))
...
---设置TPS视角鼠标显示
function camera.show_tps_mode_mouse(player,switch)
    game_api.set_tps_mode_ctrl(player.base(),switch)
end
...
function camera.linear_move_by_time(player, point, time, move_type)
    game_api.camera_linear_move_duration(player.base(), point.base(), Fix32(time), move_type)
end
...
function camera.set_camera_follow_unit(player, unit, x, y, height)
    game_api.camera_set_follow_unit(player.base(), unit.base(), Fix32(x), Fix32(y), Fix32(height))
end
...
---设置镜头第三人称跟随单位
function camera.set_tps_follow_unit(player, unit, sensitivity, yaw, pitch, x_offset, y_offset, z_offset, camera_distance)
    game_api.camera_set_tps_follow_unit(player.base(), unit.base(), Fix32(sensitivity), Fix32(yaw), Fix32(pitch), Fix32(x_offset), Fix32(y_offset), Fix32(z_offset), Fix32(camera_distance))
end
