...
function beam:set(data)
    ...
    if data.target.type == 'point' then
        ...
        game_api.set_link_sfx_point(self.light,data.point_type,data.target.base,Fix32(data.height))
    end
    if data.target.type == 'unit' then
        game_api.set_link_sfx_unit_socket(self.light,data.point_type,data.target.base,data.socket)
    end
end
