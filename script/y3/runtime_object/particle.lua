...
function particle.screen_particle(data)
    ...
    local py_particle = game_api.add_sfx_to_camera_with_return(data.type, data.time, data.target, data.is_on_fog,data.immediate)
...
function particle:set_facing(direction)
    game_api.set_sfx_angle(self.base(), Fix32(direction))
end
...
function particle:set_scale(x,y,z)
    game_api.set_sfx_scale(self.base(), Fix32(x), Fix32(y), Fix32(z))
end
...
function particle:set_height(height)
    game_api.set_sfx_height(self.base(), Fix32(height))
end
...
function particle:set_animation_speed(speed)
    game_api.set_sfx_animation_speed(self.base(), Fix32(speed))
end
...
function particle:set_time(duration)
    game_api.set_sfx_duration(self.base(), Fix32(duration))
end
