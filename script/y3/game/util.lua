...
function y3.set_fog_attribute(fog, direction, pos_x, pos_y, pos_z, scale_x, scale_y, scale_z, red, green, blue, concentration, speed)
    game_api.set_fog_attr(fog, 4095, Fix32(direction), Fix32(pos_x), Fix32(pos_y), Fix32(pos_z), Fix32(scale_x), Fix32(scale_y), Fix32(scale_z), Fix32(red), Fix32(green), Fix32(blue), Fix32(concentration), Fix32(speed))
end
...
function y3.set_fog_attr(fog,attr,value)
    game_api.set_fog_attr_new(fog,attr,value)
end
...
---@param key string 键   
---设置一维表的值
function y3.set_table_value(table, value, key)
    game_api.set_table_value(table, value, key, "", "", "", "", "")
end
...
---设置多维表的值
function y3.set_table_value_multi(table, value, key, field)
...
function y3.get_local_language_environment()
    return game_api.get_client_language_type()
end
